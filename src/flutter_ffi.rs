    ...
    if key == "is-printer-installed" {
        ...
        if key.starts_with("download-data-") {
            ...
            return match (
                ...
                (Ok(true), false) => format!("ama-connect-{_version}-x86_64.msi"),
                (Ok(true), true) | (Ok(false), _) => format!("ama-connect-{_version}-x86_64.exe"),
                (Err(e), _) => {
            ...
            #[cfg(target_os = "macos")]
            {
                return if cfg!(target_arch = "x86_64") {
                    format!("ama-connect-{_version}-x86_64.dmg")
                } else if cfg!(target_arch = "aarch64") {
                    format!("ama-connect-{_version}-aarch64.dmg")
                } else {
