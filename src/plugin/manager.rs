...
fn get_plugin_source_list() -> Vec<PluginSource> {
    ...
    // vec![PluginSource {
    //     name: "ama-connect".to_string(),
    //     url: "https://raw.githubusercontent.com/fufesou/ama-connect-plugins/main".to_string(),
...
fn elevate_install(
    ...
    crate::platform::elevate(args, "AMA Connect wants to install then plugin")
}
...
fn elevate_uninstall(plugin_id: &str) -> ResultType<bool> {
    crate::platform::elevate(
        ...
        "AMA Connect wants to uninstall the plugin",
    )
}
