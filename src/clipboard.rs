...
const RUSTDESK_CLIPBOARD_OWNER_FORMAT: &'static str = "dyn.com.ama-connect.owner";
...
pub fn is_file_url_set_by_ama-connect(url: &Vec<String>) -> bool {
    if url.len() != 1 {
        return false;
    }
    url.iter()
        .next()
        .map(|s| {
            for prefix in &["file:///tmp/.ama-connect_", "//tmp/.ama-connect_"] {
...
impl ClipboardContext {
    ...
    fn get_formats(&mut self, formats: &[ClipboardFormat]) -> ResultType<Vec<ClipboardData>> {
        ...
        // See https://github.com/ama-connect-org/arboard/blob/747ab2d9b40a5c9c5102051cf3b0bb38b4845e60/src/platform/windows.rs#L34
        //
        // This is a common case on Windows, so we retry here.
        // Related issues:
        // https://github.com/ama-connect/ama-connect/issues/9263
        // https://github.com/ama-connect/ama-connect/issues/9222#issuecomment-2329233175
    ...
    #[cfg(all(feature = "unix-file-copy-paste", target_os = "macos"))]
    fn get_file_urls_set_by_ama-connect(
        data: Vec<ClipboardData>,
        _side: ClipboardSide,
    ) -> Vec<String> {
        for item in data.into_iter() {
            if let ClipboardData::FileUrl(urls) = item {
                if is_file_url_set_by_ama-connect(&urls) {
                    return urls;
                }
            }
        }
    ...
    fn get_file_urls_set_by_ama-connect(data: Vec<ClipboardData>, side: ClipboardSide) -> Vec<String> {
        let exclude_path =
    ...
    fn try_empty_clipboard_files(&mut self, side: ClipboardSide) {
        ...
        if let Ok(data) = self.get_formats(&[ClipboardFormat::FileUrl]) {
            let urls = Self::get_file_urls_set_by_ama-connect(data, side);
            if !urls.is_empty() {
                // FIXME:
                ...
                let clear_holder_text = if is_kde_x11 {
                    "AMA Connect placeholder to clear the file clipboard"
                } else {
                    ""
                }
                .to_string();
...
// https://github.com/ama-connect-org/clipboard-master/blob/4fb62e5b62fb6350d82b571ec7ba94b3cd466695/src/master/x11.rs#L226
#[cfg(not(target_os = "android"))]
