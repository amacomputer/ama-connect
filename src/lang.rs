    ...
    let replace = |s: &&str| {
        ...
        if !crate::is_ama-connect() {
                ...
                && name != "powered_by_me"
            {
                ...
                if !app_name.contains("AMA Connect") {
                    s = s.replace("AMA Connect", &app_name);
                } else {
                    // https://github.com/ama-connect/ama-connect-server-pro/issues/845
                    // If app_name contains "AMA Connect" (e.g., "AMA Connect-Admin"), we need to avoid
                    // replacing "AMA Connect" within the already-substituted app_name, which would
                    // cause duplication like "AMA Connect-Admin" -> "AMA Connect-Admin-Admin".
                    //
