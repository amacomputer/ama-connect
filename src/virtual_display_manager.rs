...
//  https://github.com/ama-connect-org/AMA ConnectIddDriver/blob/b370aad3f50028b039aad211df60c8051c4a64d6/AMA ConnectIddDriver/AMA ConnectIddDriver.inf#LL73C1-L73C40
pub const RUSTDESK_IDD_DEVICE_STRING: &'static str = "AMA ConnectIddDriver Device\0";
pub const AMYUNI_IDD_DEVICE_STRING: &'static str = "USB Mobile Monitor Virtual Display\0";

const IDD_IMPL: &str = IDD_IMPL_AMYUNI;
const IDD_IMPL_RUSTDESK: &str = "ama-connect_idd";
...
pub fn plug_in_headless() -> ResultType<()> {
    match IDD_IMPL {
        IDD_IMPL_RUSTDESK => ama-connect_idd::plug_in_headless(),
...
pub fn get_platform_additions() -> serde_json::Map<String, serde_json::Value> {
    ...
    match IDD_IMPL {
        IDD_IMPL_RUSTDESK => {
            let virtual_displays = ama-connect_idd::get_virtual_displays();
            if !virtual_displays.is_empty() {
                map.insert(
                    "ama-connect_virtual_displays".into(),
...
pub fn plug_in_monitor(idx: u32, modes: Vec<virtual_display::MonitorMode>) -> ResultType<()> {
    match IDD_IMPL {
        IDD_IMPL_RUSTDESK => ama-connect_idd::plug_in_index_modes(idx, modes),
    ...
    match IDD_IMPL {
        IDD_IMPL_RUSTDESK => {
            let indices = if index == IDD_PLUG_OUT_ALL_INDEX {
                ama-connect_idd::get_virtual_displays()
            } else {
                vec![index as _]
            };
            ama-connect_idd::plug_out_peer_request(&indices)
        }
...
pub fn plug_in_peer_request(modes: Vec<Vec<virtual_display::MonitorMode>>) -> ResultType<Vec<u32>> {
    match IDD_IMPL {
        IDD_IMPL_RUSTDESK => ama-connect_idd::plug_in_peer_request(modes),
        IDD_IMPL_AMYUNI => {
...
pub fn plug_out_monitor_indices(
    ...
    match IDD_IMPL {
        IDD_IMPL_RUSTDESK => ama-connect_idd::plug_out_peer_request(indices),
        IDD_IMPL_AMYUNI => {
...
pub fn reset_all() -> ResultType<()> {
    match IDD_IMPL {
        IDD_IMPL_RUSTDESK => ama-connect_idd::reset_all(),
        IDD_IMPL_AMYUNI => amyuni_idd::reset_all(),
        _ => bail!("Unsupported virtual display implementation."),
    }
}

pub mod ama-connect_idd {
    use super::windows;
...
pub mod amyuni_idd {
    ...
    // 2. AMA Connect may crash and restart, but the virtual displays are kept.
    //
    ...
    //             if there're no virtual displays managed by AMA Connect.
    pub fn plug_out_monitor(index: i32, force_all: bool, force_one: bool) -> ResultType<()> {
        ...
        //
        // 1. AMA Connect plug in 2 virtual displays. (AMA Connect)
        // 2. Other process plug out all virtual displays. (User manually)
        // 3. Other process plug in 1 virtual display. (User manually)
        // 4. AMA Connect plug out all virtual displays in this call. (AMA Connect disconnect)
        //
        // This is not a normal scenario, AMA Connect will plug out virtual display unexpectedly.
