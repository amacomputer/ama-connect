// https://github.com/ama-connect/ama-connect/discussions/6444#discussioncomment-9010062
...
//*************************************************************
//
//  DeleteAMA ConnectTestCertsW_SingleHive()
//
//  Purpose:    Deletes AMA Connect Test certificates and wrong key stores
//
...
BOOL DeleteAMA ConnectTestCertsW_SingleHive(HKEY RootKey, LPWSTR Prefix = NULL) {
...
extern "C" void DeleteAMA ConnectTestCertsW() {
	// Current user
	std::wcout << "*** Current User" << std::endl;
	DeleteAMA ConnectTestCertsW_SingleHive(HKEY_CURRENT_USER);

	// Local machine (requires admin rights)
	std::wcout << "*** Local Machine" << std::endl;
	DeleteAMA ConnectTestCertsW_SingleHive(HKEY_LOCAL_MACHINE);
	...
	for (DWORD Index = 0; ; Index++) {
		...
		DeleteAMA ConnectTestCertsW_SingleHive(HKEY_USERS, SubKeyName);
	}
