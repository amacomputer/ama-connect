on run {daemon_file, agent_file, user, cur_pid, source_dir}
  ...
  set agent_plist to "/Library/LaunchAgents/com.carriez.AMA Connect_server.plist"
  set daemon_plist to "/Library/LaunchDaemons/com.carriez.AMA Connect_service.plist"
  set app_bundle to "/Applications/AMA Connect.app"
  ...
  set kill_others to "pids=$(pgrep -x 'AMA Connect' | grep -vx " & cur_pid & " || true); if [ -n \"$pids\" ]; then echo \"$pids\" | xargs kill -9 || true; fi;"
  ...
  do shell script sh with prompt "AMA Connect wants to update itself" with administrator privileges
end run
