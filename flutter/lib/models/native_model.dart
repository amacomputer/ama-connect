...
class PlatformFFI {
  ...
  AmaConnectImpl get ffiBind => _ffiBind;
  ...
  void _startListenEvent(AmaConnectImpl amaConnectImpl) {
    final appType =
        _appType == kAppTypeDesktopRemote ? '$_appType,$kWindowId' : _appType;
    var sink = amaConnectImpl.startGlobalEventStream(appType: appType);
    sink.listen((message) {
      () async {
        try {
