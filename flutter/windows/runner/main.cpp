...
{
  HINSTANCE hInstance = LoadLibraryA("libama-connect.dll");
  if (!hInstance)
  {
    std::cout << "Failed to load libama-connect.dll." << std::endl;
    return EXIT_FAILURE;
  }
  ...
  if (!c_args)
  {
    ...
    // std::cout << "AMA Connect [" << args_str << "], core returns false, exiting without launching Flutter app." << std::endl;
    return EXIT_SUCCESS;
  }
