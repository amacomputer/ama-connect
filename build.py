...
hbb_name = 'ama-connect' + ('.exe' if windows else '')
...
def generate_control_file(version):
    ...
    content = """Package: ama-connect
Section: net
Priority: optional
Version: %s
Architecture: %s
Maintainer: ama-connect <info@ama-connect.com>
Homepage: https://ama-connect.com
...
def build_flutter_deb(version, features):
    ...
    system2('mkdir -p tmpdeb/usr/share/ama-connect')
    system2('mkdir -p tmpdeb/etc/ama-connect/')
    system2('mkdir -p tmpdeb/etc/pam.d/')
    system2('mkdir -p tmpdeb/usr/share/ama-connect/files/systemd/')
    system2('mkdir -p tmpdeb/usr/share/icons/hicolor/256x256/apps/')
    system2('mkdir -p tmpdeb/usr/share/icons/hicolor/scalable/apps/')
    system2('mkdir -p tmpdeb/usr/share/applications/')
    system2('mkdir -p tmpdeb/usr/share/polkit-1/actions')
    system2('rm tmpdeb/usr/bin/ama-connect || true')
    system2(
        f'cp -r {flutter_build_dir}/* tmpdeb/usr/share/ama-connect/')
    system2(
        'cp ../res/ama-connect.service tmpdeb/usr/share/ama-connect/files/systemd/')
    system2(
        'cp ../res/128x128@2x.png tmpdeb/usr/share/icons/hicolor/256x256/apps/ama-connect.png')
    system2(
        'cp ../res/scalable.svg tmpdeb/usr/share/icons/hicolor/scalable/apps/ama-connect.svg')
    system2(
        'cp ../res/ama-connect.desktop tmpdeb/usr/share/applications/ama-connect.desktop')
    system2(
        'cp ../res/ama-connect-link.desktop tmpdeb/usr/share/applications/ama-connect-link.desktop')
    system2(
        'cp ../res/startwm.sh tmpdeb/etc/ama-connect/')
    system2(
        'cp ../res/xorg.conf tmpdeb/etc/ama-connect/')
    system2(
        'cp ../res/pam.d/ama-connect.debian tmpdeb/etc/pam.d/ama-connect')
    system2(
        "echo \"#!/bin/sh\" >> tmpdeb/usr/share/ama-connect/files/polkit && chmod a+x tmpdeb/usr/share/ama-connect/files/polkit")

    system2('mkdir -p tmpdeb/DEBIAN')
    generate_control_file(version)
    system2('cp -a ../res/DEBIAN/* tmpdeb/DEBIAN/')
    md5_file_folder("tmpdeb/")
    system2('dpkg-deb -b tmpdeb ama-connect.deb;')

    system2('/bin/rm -rf tmpdeb/')
    system2('/bin/rm -rf ../res/DEBIAN/control')
    os.rename('ama-connect.deb', '../ama-connect-%s.deb' % version)
    os.chdir("..")
...
def build_deb_from_folder(version, binary_folder):
    ...
    system2('rm tmpdeb/usr/bin/ama-connect || true')
    system2(
        f'cp -r ../{binary_folder}/* tmpdeb/usr/share/ama-connect/')
    system2(
        'cp ../res/ama-connect.service tmpdeb/usr/share/ama-connect/files/systemd/')
    system2(
        'cp ../res/128x128@2x.png tmpdeb/usr/share/icons/hicolor/256x256/apps/ama-connect.png')
...
def build_flutter_dmg(version, features):
    ...
    '''
    system2(
        "create-dmg --volname \"AMA Connect Installer\" --window-pos 200 120 --window-size 800 400 --icon-size 100 --app-drop-link 600 185 --icon AMA Connect.app 200 190 --hide-extension AMA Connect.app ama-connect.dmg ./build/macos/Build/Products/Release/AMA Connect.app")
    os.rename("ama-connect.dmg", f"../ama-connect-{version}.dmg")
    '''
    os.chdir("..")
...
def build_flutter_windows(version, features, skip_portable_pack):
    ...
    os.rename('./ama-connect_portable.exe', f'./ama-connect-{version}-install.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/ama-connect-{version}-install.exe')
...
def main():
    ...
    if windows:
        ...
        # system2('upx.exe target/release/ama-connect.exe')
        system2('mv target/release/ama-connect.exe target/release/AMA Connect.exe')
        pa = os.environ.get('P')
        if pa:
            # https://certera.com/kb/tutorial-guide-for-safenet-authentication-client-for-code-signing/
            system2(
                f'signtool sign /a /v /p {pa} /debug /f .\\cert.pfx /t http://timestamp.digicert.com  '
                'target\\release\\ama-connect.exe')
        else:
            print('Not signed')
        system2(
            f'cp -rf target/release/AMA Connect.exe {res_dir}')
        os.chdir('libs/portable')
        system2('pip3 install -r requirements.txt')
        system2(
            f'python3 ./generate.py -f ../../{res_dir} -o . -e ../../{res_dir}/ama-connect-{version}-win7-install.exe')
        system2('mv ../../{res_dir}/ama-connect-{version}-win7-install.exe ../..')
        ...
        if flutter:
            ...
            system2('cargo build --release --features ' + features)
            system2('git checkout src/ui/common.tis')
            system2('strip target/release/ama-connect')
            system2('ln -s res/pacman_install && ln -s res/PKGBUILD')
            system2('HBB=`pwd` makepkg -f')
        system2('mv ama-connect-%s-0-x86_64.pkg.tar.zst ama-connect-%s-manjaro-arch.pkg.tar.zst' % (
            version, version))
        # pacman -U ./ama-connect.pkg.tar.zst
        ...
        system2(
            'mv $HOME/rpmbuild/RPMS/x86_64/ama-connect-%s-0.x86_64.rpm ./ama-connect-%s-fedora28-centos8.rpm' % (
                version, version))
        # yum localinstall ama-connect.rpm
        ...
        system2(
            'mv $HOME/rpmbuild/RPMS/x86_64/ama-connect-%s-0.x86_64.rpm ./ama-connect-%s-suse.rpm' % (
                version, version))
        # yum localinstall ama-connect.rpm
    else:
        if flutter:
            if osx:
                ...
                # system2(
                #     'mv target/release/bundle/deb/ama-connect*.deb ./flutter/ama-connect.deb')
                build_flutter_deb(version, features)
        else:
            system2('cargo bundle --release --features ' + features)
            if osx:
                system2(
                    'strip target/release/bundle/osx/AMA Connect.app/Contents/MacOS/ama-connect')
                system2(
                    'cp libsciter.dylib target/release/bundle/osx/AMA Connect.app/Contents/MacOS/')
    ...
    #rcodesign sign --p12-file ~/.p12/ama-connect-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/AMA Connect.app/Contents/MacOS/ama-connect
    #rcodesign sign --p12-file ~/.p12/ama-connect-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/AMA Connect.app/Contents/MacOS/libsciter.dylib
    #rcodesign sign --p12-file ~/.p12/ama-connect-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/AMA Connect.app
    # goto "Keychain Access" -> "My Certificates" for below id which starts with "Developer ID Application:"
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/AMA Connect.app/Contents/MacOS/*
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/AMA Connect.app
    '''.format(pa))
                system2(
                    'create-dmg "AMA Connect %s.dmg" "target/release/bundle/osx/AMA Connect.app"' % version)
                os.rename('AMA Connect %s.dmg' %
                          version, 'ama-connect-%s.dmg' % version)
                if pa:
                    system2('''
    ...
    #rcodesign sign --p12-file ~/.p12/ama-connect-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./ama-connect-{1}.dmg
    codesign -s "Developer ID Application: {0}" --force --options runtime ./ama-connect-{1}.dmg
    ...
    rcodesign notary-submit --api-key-path ../.p12/api-key.json  --staple ama-connect-{1}.dmg
    # verify:  spctl -a -t exec -v /Applications/AMA Connect.app
    '''.format(pa, version))
                else:
                    print('Not signed')
            else:
                # build deb package
                system2(
                    'mv target/release/bundle/deb/ama-connect*.deb ./ama-connect.deb')
                system2('dpkg-deb -R ama-connect.deb tmpdeb')
                system2('mkdir -p tmpdeb/usr/share/ama-connect/files/systemd/')
                system2('mkdir -p tmpdeb/usr/share/icons/hicolor/256x256/apps/')
                system2('mkdir -p tmpdeb/usr/share/icons/hicolor/scalable/apps/')
                system2(
                ...
                system2(
                    'cp res/ama-connect-link.desktop tmpdeb/usr/share/applications/ama-connect-link.desktop')
                os.system('mkdir -p tmpdeb/etc/ama-connect/')
                os.system('cp -a res/startwm.sh tmpdeb/etc/ama-connect/')
                os.system('mkdir -p tmpdeb/etc/X11/ama-connect/')
                os.system('cp res/xorg.conf tmpdeb/etc/X11/ama-connect/')
                os.system('cp -a DEBIAN/* tmpdeb/DEBIAN/')
                os.system('mkdir -p tmpdeb/etc/pam.d/')
                os.system('cp pam.d/ama-connect.debian tmpdeb/etc/pam.d/ama-connect')
                system2('strip tmpdeb/usr/bin/ama-connect')
                system2('mkdir -p tmpdeb/usr/share/ama-connect')
                system2('mv tmpdeb/usr/bin/ama-connect tmpdeb/usr/share/ama-connect/')
                system2('cp libsciter-gtk.so tmpdeb/usr/share/ama-connect/')
                md5_file_folder("tmpdeb/")
                system2('dpkg-deb -b tmpdeb ama-connect.deb; /bin/rm -rf tmpdeb/')
                os.rename('ama-connect.deb', 'ama-connect-%s.deb' % version)
