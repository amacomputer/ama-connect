...
1. Use a temporary file `/tmp/ama-connect_<uuid>` as a placeholder in the pasteboard.
...
2. Create temp file in `/tmp/.ama-connect_*`.

### `paste_observer.rs`

Use `fsevent` to observe the paste operation with the source file `/tmp/.ama-connect_*`.
